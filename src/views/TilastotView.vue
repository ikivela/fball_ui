          ...
          <div class="col-12">
            <div v-if="!currentSeason" class="no-selection">
              <div class="no-selection-content">
                ...
                <h3>Valitse kausi</h3>
                <p>Valitse kausi yllä olevista suodattimista nähdäksesi tilastot.</p>
              </div>
            </div>
            ...
            <div v-else class="stats-content">
              <div v-for="key in seasonStats" :key="`${key.season}-${key.class}`" class="stats-card">
                ...
                <div class="stats-table">
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        <tr v-for="(player, index) in key.stats" :key="player.name" class="player-row">
                          ...
                          <td class="player-penalties">
                            <span class="stat-value penalties">{{ player.penalties || 0 }}</span>
                          </td>
                          <td class="player-total">
                            <span class="stat-value total">{{ player.total || 0 }}</span>
                          </td>
                        </tr>
                      </tbody>
                    </table>
  ...
  computed: {
    seasonbuttons() {
      if (this.allStats.length > 0) {
        return [...new Set(this.allStats.map((item) => item.season))].sort().reverse();
      }
      return [];
    },
    ...
    totalPlayers() {
      return this.seasonStats.reduce((total, stat) => total + (stat.stats ? stat.stats.length : 0), 0);
    },
    
    totalGoals() {
      return this.seasonStats.reduce((total, stat) => {
        if (stat.stats) {
          return total + stat.stats.reduce((sum, player) => sum + (player.goals || 0), 0);
        }
        return total;
      }, 0);
    },
...
// Stats Section
.stats-section {
  .no-selection, .no-stats {
    padding: 3rem 1rem;
    text-align: center;
    
    .no-selection-content, .no-stats-content {
      i {
        font-size: 3rem;
