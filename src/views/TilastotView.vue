<template>
  <div>
    <b-container>
      <p>
        Tilastot ovat vielä kehityksen alla, ja puutteellisia tietoja löytyy
        varmasti...
      </p>
      <b-row
        ><b-button
          ...
          v-bind:key="button"
          @click="setSeason(button)"
          v-for="button in seasonbuttons"
          variant-outlier="primary"
          >{{ button }}</b-button
        ></b-row
      >
      <b-row
      ...
      <div v-if="currentSeason">
        <b-card
          ...
          v-bind:key="`${key.season}-${key.class}`"
        >
